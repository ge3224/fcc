...
function orbitalPeriod(arr) {
  ...
  return arr.map((satellite) => {
    const a = earthRadius + satellite.avgAlt;
    return {
      name: satellite.name,
      orbitalPeriod: Math.round(2 * Math.PI * Math.sqrt(Math.pow(a, 3) / GM)),
    }
  });
}
...
function ch21Handler() {
  const tests = [
    assertion(orbitalPeriod([{ name: "sputnik", avgAlt: 35873.5553 }]), [{ name: "sputnik", orbitalPeriod: 86400 }]),
    assertion(orbitalPeriod([{ name: "iss", avgAlt: 413.6 }, { name: "hubble", avgAlt: 556.7 }, { name: "moon", avgAlt: 378632.553 }]), [{ name: "iss", orbitalPeriod: 5557 }, { name: "hubble", orbitalPeriod: 5734 }, { name: "moon", orbitalPeriod: 2377399 }]),
  ];
